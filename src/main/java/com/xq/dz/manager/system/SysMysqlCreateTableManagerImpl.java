package com.xq.dz.manager.system;
...
import com.xq.dz.annotation.Column;
import com.xq.dz.annotation.LengthCount;
import com.xq.dz.annotation.Table;
import com.xq.dz.command.CreateTableParam;
import com.xq.dz.command.SysMysqlColumns;
import com.xq.dz.constants.MySqlTypeConstant;
import com.xq.dz.dao.system.CreateMysqlTablesMapper;
import com.xq.dz.utils.ClassTools;

/**
 * 项目启动时自动扫描配置的目录中的model，根据配置的规则自动创建或更新表 该逻辑只适用于mysql，其他数据库尚且需要另外扩展，因为sql的语法不同
 *
 * @author arvin
 * @version 2018-1-23 10:43:06
 */
@Transactional
...
public class SysMysqlCreateTableManagerImpl implements SysMysqlCreateTableManager {
	...
	@Value("#{configProperties['mybatis.db.names']}")
	...
	public void createMysqlTable() {
		...
		if (StringUtils.isNotEmpty(dbNames)) {
			for (String dbName : dbNames.split(";")) {
				int count = this.createMysqlTablesMapper.findDataBaseCountByDBName(dbName);
				if (count <= 0) {
					this.createMysqlTablesMapper.createDataBase(dbName);
				}
		...
		// 构建出全部表的增删改的map
		allTableMapConstruct(dbNameList, mySqlTypeAndLengthMap, classes);
	}

	/**
	 * 构建出全部表的增删改的map
	 * 
	 * @param dbNameList 数据库的个数
	 ...
	 */
	private void allTableMapConstruct(List<String> dbNameList, Map<String, Object> mySqlTypeAndLengthMap, Set<Class<?>> classes) {
		for (Class<?> clas : classes) {
			...
			// 先查该表是否以存在
			int dbSize = table.dbSize() >= dbNameList.size() ? dbNameList.size() : table.dbSize();
			int tableSize = table.tableSize();
			// 确保单库的时候，采用最后一个库的原则
			int i = dbNameList.size() - dbSize;
			for (; i < dbNameList.size(); i++) {
				for (int j = 0; j < tableSize; j++) {
					...
					String tableName = "";
					String tableNameNo = "";
					String dbNameNo = dbNameList.get(i);
					String dbName = "`" + dbNameNo + "`";
					if (tableSize == 1) {
						tableNameNo = table.name();
					} else {
						tableNameNo = table.name() + "_" + j;
					}
					tableName = "`" + tableNameNo + "`";
					// 跨库操作时规则，统一提出来，无需在mapper层再次整理
					String finalTableName = dbName + "." + tableName;
					int exist = createMysqlTablesMapper.findTableCountByTableName(dbNameNo, tableNameNo);
					// 用于存新增表的字段
					...
					// 不存在时
					if (exist == 0) {
						newTableMap.put(finalTableName, newFieldList);
					} else {
						// 如果配置文件配置的是create，表示将所有的表删掉重新创建
						if ("create".equals(tableAuto)) {
							createMysqlTablesMapper.dorpTableByName(finalTableName);
							newTableMap.put(finalTableName, newFieldList);
						} else if ("update".equals(tableAuto)) {
							// 已存在时理论上做修改的操作，这里查出该表的结构
							List<SysMysqlColumns> tableColumnList = createMysqlTablesMapper
									.findTableEnsembleByTableName(dbNameNo, tableNameNo);
							...
							// 2. 找出删除的字段
							// 3. 找出更新的字段
							buildAddAndRemoveAndModifyFields(mySqlTypeAndLengthMap, modifyTableMap, addTableMap,
									removeTableMap, dropKeyTableMap, dropUniqueTableMap, finalTableName, newFieldList,
									removeFieldList, addFieldList, modifyFieldList, dropKeyFieldList,
									dropUniqueFieldList, tableColumnList, columnNames);
						}
					}
					// 根据传入的map，分别去创建或修改表结构
					createOrModifyTableConstruct(newTableMap, modifyTableMap, addTableMap, removeTableMap,
	...
	private void tableFieldsConstruct(Map<String, Object> mySqlTypeAndLengthMap, Class<?> clas,
		...
		for (Field field : fields) {
			...
			if (hasAnnotation) {
				...
				// 主键或唯一键时设置必须不为null
				if (column.isKey()) {
					primaryKeySb.append(column.name() + ",");
					param.setFieldIsNull(false);
				} else if (column.isUnique()) {
					uniqueSb.append(column.name() + ",");
					param.setFieldIsNull(false);
				} else {
		...
		// 主键、唯一键的sql拼装
		if (primaryKeySb.length() > 0 || uniqueSb.length() > 0) {
			newFieldList.remove(newFieldList.size() - 1);
			if (primaryKeySb.length() > 0) {
				param.setPrimaryKey(primaryKeySb.substring(0, primaryKeySb.length() - 1));
			}
			if (uniqueSb.length() > 0) {
				param.setUniqueKey(uniqueSb.substring(0, uniqueSb.length() - 1));
			}
			newFieldList.add(param);
		}
	}
